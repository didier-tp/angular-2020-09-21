...
export class ConversionPage {
  navigateTo(): Promise<unknown> {
    return browser.get(browser.baseUrl+"/conversion") as Promise<unknown>;
  }
  ...
  getSelectCodeDevSourceOptionElementContaining(val : string): ElementFinder {
    return element(by.cssContainingText("app-root app-conversion select[name='codeDevSource'] option",val)) ;
  }

  getSelectCodeDevCibleOptionElementContaining(val : string): ElementFinder {
    return element(by.cssContainingText("app-root app-conversion select[name='codeDevCible'] option",val)) ;
  }
}
